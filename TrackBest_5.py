...
class StrictLockedTracker:
    ...
    """
    def __init__(self, color_threshold=COLOR_DISTANCE_THRESHOLD, color_feat = None, lost_count=0):
        self.locked = False
        ...
        if self.is_lost():
            ...
            for c in candidates:
                ...
                if move_dev > max_move:
                    continue
                t = move_dev
                if color_filtered is None or t < best_relaxed_score:
...
def process_video(input_path: str, output_path: str, highlight_path: str,
    ...
    tracker = StrictLockedTracker(color_threshold=COLOR_DISTANCE_THRESHOLD, color_feat=color_feat0, lost_count=0)
    ...
    while True:
        ...
        if tracker.is_lost():
            ...
            tracker = StrictLockedTracker(color_threshold=COLOR_DISTANCE_THRESHOLD, color_feat=color_feat0, lost_count=tracker.lost_count)
            out_full.write(frame)
            if pbar: pbar.update(1)
            continue
